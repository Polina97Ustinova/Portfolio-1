...
print('correlation:')
print(dp.corrcoef(int(df['Current_player']), int(df['Hours_played'])))
...
df[["Peak_players","Hours_played"]] = df[[ "Peak_players","Hours_played"]].apply(lambda x: x.str.replace(',',''))
df = df.astype({'Peak_players': 'float64',
...
#Checking if all the columns are in order
print(df['Hours Played'])


df.sort_values(['Hours Played'], 
axis=0, 
ascending=[False],
inplace=True)
